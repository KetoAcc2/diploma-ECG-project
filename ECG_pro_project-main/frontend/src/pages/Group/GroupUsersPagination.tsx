...
const GroupUsersPagination: FC<GroupUsersPropWrapper> = ({
  genericTable,
  headers,
  groupId,
}: GroupUsersPropWrapper) => {
  ...
  const removeUserFromGroupMutation: UseMutationResult<
    void,
    Error,
    RemoveUserFromGroupDTO
  > = useMutation<void, Error, RemoveUserFromGroupDTO>(
    async ({ groupId, userId }) => {
      await removeUserFromGroup({
        groupId: groupId,
        userId: userId,
      } as RemoveUserFromGroupDTO);
      await queryClient.invalidateQueries("usersFromGroup");
    },
    {
      onMutate: () => {},
      onError: () => {},
                    ...
                    <>
                      <TableCell
                        ...
                        <Button
                          ...
                          component={Link}
                          variant="contained"
                          to={`${genericTable.detailsUrl}/${row.columns[0]}/${groupId}`}>
                          {t("detail.text")}
                        </Button>
                      </TableCell>
                      <TableCell
                        ...
                        <Button
                          ...
                          onClick={async (e) => {
                            ...
                            if (confirmed) {
                              ...
                              e.persist();
                              queryClient.refetchQueries("usersFromGroup");
                            }
                          }}>
                          {t("remove.text")}
                        </Button>
...
export default GroupUsersPagination;
